  ...
  <div class="dashboard-container">
    ...
    <div class="approvals-container">
      ...
      <CollapsibleTable
        ...
        :on-next-page="() => handlePendingPageChange(pendingPage + 1)"
        :on-prev-page="() => handlePendingPageChange(pendingPage - 1)"
      ...
      <CollapsibleTable
        ...
        :on-next-page="() => handleApprovedPageChange(approvedPage + 1)"
        :on-prev-page="() => handleApprovedPageChange(approvedPage - 1)"
      ...
      <CollapsibleTable
        ...
        :on-next-page="() => handleRejectedPageChange(rejectedPage + 1)"
        :on-prev-page="() => handleRejectedPageChange(rejectedPage - 1)"
    ...
    <div v-if="showFrameworkDetails && selectedApproval" class="framework-details-modal">
      <div class="framework-details-content">
        <span class="close-x" @click="closeApprovalDetails" title="Close">&times;</span>
        <h3>
      ...
      <div v-if="showRejectModal" class="reject-modal">
        <div class="reject-modal-content">
          <span class="close-x" @click="cancelRejection" title="Close">&times;</span>
    ...
    <div class="rejected-approvals-listt" v-if="rejectedFrameworks.length">
...
export default {
  ...
  methods: {
    ...
    handlePendingPageSizeChange(size) {
    ...
    handleApprovedPageSizeChange(size) {
      this.approvedPageSize = size;
      this.approvedPage = 1;
    },
    handleRejectedPageChange(page) {
      this.rejectedPage = page;
    },
    handleRejectedPageSizeChange(size) {
  ...
  computed: {
    ...
    pendingSectionConfig() {
      // Map first, then paginate
      const mapped = this.pendingFrameworks.map(framework => this.transformFrameworkForTable(framework));
      const start = (this.pendingPage - 1) * this.pendingPageSize;
      const end = start + this.pendingPageSize;
      return {
        name: 'Pending Review',
        statusClass: 'pending',
        tasks: mapped.slice(start, end)
      };
    },
    approvedSectionConfig() {
      const mapped = this.approvedFrameworks.map(framework => this.transformFrameworkForTable(framework));
    ...
    rejectedSectionConfig() {
      const mapped = this.rejectedFrameworksForTable.map(framework => this.transformFrameworkForTable(framework));
      const start = (this.rejectedPage - 1) * this.rejectedPageSize;
    ...
    // Pagination objects for CollapsibleTable
    pendingPagination() {
      return {
        currentPage: this.pendingPage,
        totalPages: Math.max(1, Math.ceil(this.pendingFrameworks.length / this.pendingPageSize)),
        pageSize: this.pendingPageSize,
        totalCount: this.pendingFrameworks.length,
        pageSizeOptions: [6, 15, 30, 50],
        onPageSizeChange: this.handlePendingPageSizeChange
      };
    },
    approvedPagination() {
      return {
        currentPage: this.approvedPage,
        totalPages: Math.max(1, Math.ceil(this.approvedFrameworks.length / this.approvedPageSize)),
        pageSize: this.approvedPageSize,
        totalCount: this.approvedFrameworks.length,
        pageSizeOptions: [6, 15, 30, 50],
        onPageSizeChange: this.handleApprovedPageSizeChange
      };
    },
    rejectedPagination() {
      return {
        currentPage: this.rejectedPage,
        totalPages: Math.max(1, Math.ceil(this.rejectedFrameworksForTable.length / this.rejectedPageSize)),
        pageSize: this.rejectedPageSize,
        totalCount: this.rejectedFrameworksForTable.length,
        pageSizeOptions: [6, 15, 30, 50],
        onPageSizeChange: this.handleRejectedPageSizeChange
      };
    }
  },
}

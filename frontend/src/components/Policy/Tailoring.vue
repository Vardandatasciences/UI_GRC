<template>
  <div class="tailoring-container" @click="closeAllEntityDropdowns">
    ...
    <!-- Framework Dropdown (remains at the top) -->
    ...
    <!-- Framework Form Card (separate from dropdown container) -->
    <div v-if="!showPolicyDropdown && selectedFramework" class="framework-card">
      <form class="form-section framework-form" @submit.prevent="submitFrameworkForm">
            <div class="form-row">
    ...
    <!-- Policy Stepper/Tabbed Navigation Card (no framework tab) -->
    <div v-if="showStepper" class="policies-card">
      <div class="stepper policies-stepper">
        <div class="stepper-tabs-scroll">
          <div
            v-for="(tab, idx) in stepTabs"
            :key="tab.key"
            :class="['step policy-step', { active: stepIndex === idx }]"
            @click="stepIndex = idx"
        ...
        <button v-if="stepTabs.length > 0 && !showPolicyDropdown" class="add-btn add-policy-btn" @click="addPolicy" title="Add a new policy to this framework">+ Add Policy</button>
      </div>
      <!-- Only one main container for the policy form, only show the active policy -->
      <form v-if="stepIndex > 0 && policiesData[stepIndex - 1] && !showPolicyDropdown" class="policy-form-container form-section policy-form" @submit.prevent="submitFrameworkForm">
    ...
    <!-- Subpolicy Stepper/Tabbed Navigation Card (separate) -->
    <div v-if="stepIndex > 0 && policiesData[stepIndex - 1] && !showPolicyDropdown" class="subpolicies-card">
      <div class="stepper subpolicies-stepper">
        <div class="stepper-tabs-scroll">
          <div
            v-for="(sub, subIdx) in policiesData[stepIndex - 1].subPolicies"
            :key="subIdx"
            :class="['step subpolicy-step', { active: subIdx === 0 }]"
          >
            Subpolicy {{ subIdx + 1 }}
          </div>
        </div>
        <button type="button" class="add-btn add-subpolicy-btn" @click="addSubPolicy(stepIndex - 1)" title="Add a new sub-policy to this policy">+ Add Sub Policy</button>
      </div>
  ...
  setup() {
    ...
    const onFrameworkDropdown = async () => {
      ...
      try {
        ...
        stepIndex.value = policiesData.value.length > 0 ? 1 : 0
    ...
    const stepTabs = computed(() => {
      // Only show policy tabs, not the framework tab

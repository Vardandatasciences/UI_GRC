        ...
        <thead>
          <tr>
            ...
            <th 
              ...
              :style="{ width: columnWidths[column.key] ? columnWidths[column.key] + 'px' : (column.headerStyle && column.headerStyle.width ? column.headerStyle.width : 'auto') }"
            >
              <div class="column-header" style="position: relative;">
                <span>{{ column.label }}</span>
                <span v-if="column.sortable" class="filter-arrows">
                  <button 
                    class="sort-btn"
                    v-if="sortKey === column.key && sortOrder === 'asc'"
                    @click="sortByDesc(column.key)"
                ...
                <div 
                  ...
                  @touchstart="startResize($event, column.key)"
                ></div>
              </div>
            </th>
            ...
            >
              ...
              <template v-else>
                ...
                <div v-else class="text-cell">
                  <div 
                    class="cell-content"
                    :class="{ 'truncated': isTextTruncated(row[column.key], column) }"
                    :title="isTextTruncated(row[column.key], column) ? row[column.key] : ''"
                  >
                    {{ row[column.key] }}
                  </div>
                </div>
...
export default {
  ...
  return 'progress-bar-default';
    ...
    startResize(e, columnKey) {
      e.preventDefault();
      this.resizing.active = true;
      this.resizing.columnKey = columnKey;
      this.resizing.startX = e.type.startsWith('touch') ? e.touches[0].clientX : e.clientX;
      this.resizing.startWidth = this.columnWidths[columnKey] || 150;
    },
    onResizeMove(e) {
      if (!this.resizing.active) return;
      const clientX = e.type.startsWith('touch') ? e.touches[0].clientX : e.clientX;
      const delta = clientX - this.resizing.startX;
      let newWidth = this.resizing.startWidth + delta;
      if (newWidth < 50) newWidth = 50;
      this.columnWidths[this.resizing.columnKey] = newWidth;
    },
    onResizeEnd() {
      if (this.resizing.active) {
        this.resizing.active = false;
        this.resizing.columnKey = null;
      }
    }
  }
}
...
.dynamic-table th {
  ...
  border-bottom: 1px solid var(--dynamic-table-border-color, #e0e0e0);
